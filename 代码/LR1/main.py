...
class Ui_MainWindow(object):
    ...
    def setupUi(self, MainWindow, cfg, family, parseRst,tokens):
        ...
        button1widget = QtWidgets.QWidget(MainWindow)
        button1widget.setObjectName("button1widget")
        button1widget.resize(200, 600)
        ...
        self.okButton = QPushButton(button4widget)
        self.okButton.setText('词法分析')
        self.okButton.setEnabled(False)

        self.interpreterButton = QPushButton(button4widget)
        self.interpreterButton.setText('解释执行')
        ...
        self.vboxcode.addWidget(self.interpreterButton)
        self.vboxcode.addWidget(button1widget)
        # 将按钮和信号关联
        ...
        self.interpreterButton.clicked.connect(self.interpreterTest)
    ...
    def GramTest(self):
        ...
        self.interpreterButton.setEnabled(True)
        # 按下“语法分析”的按钮后，这三张表的按钮由灰变可选，点击后能查看以下三张表


        # -------------- 以上   是程序相关 ---------------------------------

    def interpreterTest(self):
        table_name = []
        for dir in self.ana.symbolTable:
            if dir.function =='main':
                table_name.append(dir.place)
        print(table_name)
        self.mdcodename = 'middleCodeFile.txt'
        self.quadruples_sequence = read_quadruples(self.mdcodename)
        self.var=execute_quadruples(self.quadruples_sequence)
        InterpDialog = QDialog(self.MainWindow)
        InterpDialog.resize(900,600)
        table = QTableWidget(InterpDialog)
        table.setColumnCount(3)
        table.setRowCount(len(table_name))
        table.setHorizontalHeaderLabels(['符号的标识符', '对应的中间变量', '最终计算结果'])
        ...
        for var_name, var_value in self.var.items():

            # print(self.ana.symbolTable)

            if var_name in table_name:
                cnt_col = 0
                for dir in self.ana.symbolTable:
                    if dir.place == var_name:
                        table.setItem(cnt_row, cnt_col, QTableWidgetItem(dir.name))
                cnt_col += 1
                table.setItem(cnt_row, cnt_col, QTableWidgetItem(var_name))
                cnt_col += 1
                table.setItem(cnt_row, cnt_col, QTableWidgetItem(str(var_value)))
                cnt_row += 1

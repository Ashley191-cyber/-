...
class CFG():
    ...
    def calFristSet(self):
        ...
        for symbol in self.NonTerminalSymbols:
            self.calNTFristSetImprove(symbol)

        return

    # 计算所有单个NT符号的Frist集，迭代生成
                ...
                for right in prod.right:
                    ...
                    if right['type'] in self.firstSet.keys():
                        ...
                        if right['type'] == symbol:
                            break
                        tempRstSet = self.calNTFristSet(right['type'])
    ...
    def calNTFristSetImprove(self, symbol):
        eps = {'class':'T','name':'','type':self.Epsilon}
        # 若 X∈VT，则FIRST(X)={X}。
        hasEpsAllBefore = -1
        prods = [prod for prod in self.prods if prod.left == symbol]
                ...
                for right in prod.right:
                    ...
                    if len(self.firstSet[right['type']]) == 0:
                        if right['type'] != symbol: #防止陷入死循环
                            self.calNTFristSetImprove(right['type'])
    ...
    #定义了scan_line(line)函数将一行的词法单元识别，并将判别词法单元的结果存储到一个list作为返回结果中。
        ...
        if nt == 'program':
            ...
            # id ( actualParaList )
                ...
                for node in n.stack: # 实参列表
                    ...
                    n.code.append(('Par', '_', '_', node_result))
                n.code.append(('call', '_', function.label, len(function.params)))
